...
export function EditUserModal({ open, onClose, user }: EditUserModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateUser.mutateAsync({ username: user.login, input: cleanInput });
      showToast('success', `사용자 "${user.login}" 정보가 수정되었습니다.`);
      handleClose();
    } catch (error) {
  ...
  return (
    <Modal open={open} onClose={handleClose} title={`사용자 수정: ${user.login}`} maxWidth="md">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-3 pt-1">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-sm text-gray-700">제한된 계정</span>
            </label>
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-sm text-gray-700">비밀번호 변경 요구</span>
            </label>
          </div>
        </div>

        <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100">
          <Button type="button" variant="secondary" onClick={handleClose} disabled={updateUser.isPending}>
            취소
          </Button>
